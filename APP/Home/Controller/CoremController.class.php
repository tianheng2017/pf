...
class CoremController extends Controller
{
    ...
    public function PaySubmitDo(){
        if($_POST){
			...
			if(($type!=1 && $type!=2 && $type!=3) || !is_numeric($total_fee) || $total_fee<=0 || $token!=$tokenv || $time<1 || empty($out_trade_no)){
				$rdata = array();
				$rdata['code'] = -1;
				$rdata['id'] = -1;
        ...
        if($_GET){
			...
			if($roborder){
				...
				if($roborder['status']==2){
					...
					$ewmlist = M('ewm')->where(array('uid'=>$olist['uid'],'ewm_price'=>$olist['price'],'ewm_class'=>$olist['class']))->find();
